#!/usr/bin/env node
/**
 * Country Pages Generator
 * This script generates all country pages with dynamic pricing from KV storage
 */
...
const countries = {
  'gb': { name: 'United Kingdom', slug: 'send-sms-united-kingdom' },
  'de': { name: 'Germany', slug: 'send-sms-germany' },
  'fr': { name: 'France', slug: 'send-sms-france' },
  'es': { name: 'Spain', slug: 'send-sms-spain' },
};

// Template for country pages (excluding Estonia which is already created)
function generateCountryPageTemplate(countryCode, countryName) {
  return `---
...
import { fetchPricingData, getCountryPricing, type CountryData } from '../../../utils/kv-pricing';
import { getCountryConfig } from '../../../utils/country-data';
...
// Combine config with pricing data
const country: CountryData = {
  ...countryConfig,
  providers,
  startingPrice: providers.length > 0 ? Math.min(...providers.map(p => p.price)) : 0,
};
...
>
  <head>
    ...
    <link rel="canonical" href={\`https://smspm.com/en/country/send-sms-\${country.name.toLowerCase().replace(/\\s+/g, '-')}\`} />
    
    <!-- Structured Data -->
    <script type="application/ld+json" is:inline set:html={JSON.stringify(structuredData)} />
    
    <!-- Hreflang for multilingual support -->
    <link rel="alternate" hreflang="en" href={\`https://smspm.com/en/country/send-sms-\${country.name.toLowerCase().replace(/\\s+/g, '-')}\`} />
    <link rel="alternate" hreflang="x-default" href={\`https://smspm.com/en/country/send-sms-\${country.name.toLowerCase().replace(/\\s+/g, '-')}\`} />
  ...
  <footer class="footer bg-gray-900 text-gray-400 py-16 pt-20">
    <div class="container mx-auto px-8">
      <div class="footer-content grid grid-cols-1 md:grid-cols-5 gap-8 mb-12">
        <div class="footer-column md:col-span-2">
          ...
          <div class="footer-social flex gap-4">
            <a href="#" class="social-link w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center text-gray-400 hover:text-cyan hover:bg-gray-700 transition-all duration-150">
        ...
        <div class="footer-column">
          ...
          <ul class="footer-links space-y-3">
            ...
            <li><a href="/integrations" class="footer-link text-gray-400 hover:text-cyan transition-colors duration-150">Integrations</a></li>
            <li><a href="/changelog" class="footer-link text-gray-400 hover:text-cyan transition-colors duration-150">Changelog</a></li>
          </ul>
        </div>
        ...
        <div class="footer-column">
          ...
          <ul class="footer-links space-y-3">
            ...
            <li><a href="/careers" class="footer-link text-gray-400 hover:text-cyan transition-colors duration-150">Careers</a></li>
            <li><a href="/press" class="footer-link text-gray-400 hover:text-cyan transition-colors duration-150">Press Kit</a></li>
        ...
        <div class="footer-column">
          ...
          <ul class="footer-links space-y-3">
            ...
            <li><a href="/security" class="footer-link text-gray-400 hover:text-cyan transition-colors duration-150">Security</a></li>
            <li><a href="/compliance" class="footer-link text-gray-400 hover:text-cyan transition-colors duration-150">Compliance</a></li>
            <li><a href="/gdpr" class="footer-link text-gray-400 hover:text-cyan transition-colors duration-150">GDPR</a></li>
          </ul>
        </div>
      </div>
      ...
      <div class="footer-bottom border-t border-gray-800 pt-8 flex flex-col md:flex-row justify-between items-center">
        ...
        <div class="footer-badges flex items-center gap-6">
      ...
      button.addEventListener('click', function() {
        ...
        if (codeBlock) {
          navigator.clipboard.writeText(codeBlock.textContent || '').then(() => {
            ...
            this.innerHTML = '<svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke="currentColor" stroke-width="1.5"/></svg>Copied!';
...
function generateAllCountryPages() {
  const pagesDir = join(__dirname, '../src/pages/en/country');
  ...
  Object.entries(countries).forEach(([countryCode, { name, slug }]) => {
    const template = generateCountryPageTemplate(countryCode, name);
    ...
    try {
      ...
      console.error(`❌ Failed to generate ${slug}.astro:`, error);
    }
  });
}
...
generateAllCountryPages();
console.log('✅ All country pages generated successfully!');